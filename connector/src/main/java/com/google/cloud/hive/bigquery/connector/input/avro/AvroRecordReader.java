...
public class AvroRecordReader implements RecordReader<NullWritable, ObjectWritable> {
  ...
  public AvroRecordReader(BigQueryInputSplit inputSplit, Configuration conf) {
    ReadRowsHelper readRowsHelper = inputSplit.getReadRowsHelper(conf);
  ...
  private Object serializeRow(GenericRecord record) {
    Schema actualSchema = AvroUtils.getSchemaInfo(record.getSchema()).actualSchema;
