...
public class AvroUtils {
  ...
  public static AvroSchemaInfo getSchemaInfo(Schema fieldSchema) {
    AvroSchemaInfo schemaInfo = new AvroSchemaInfo();
    schemaInfo.actualSchema = fieldSchema;
    // Check if field is nullable, which is represented as an UNION of NULL and primitive type.
    schemaInfo.nullable = false;
    if (schemaInfo.actualSchema.getType() == Type.UNION) {
      if (fieldSchema.getTypes().size() == 2) {
        if (fieldSchema.getTypes().get(0).getType() == Type.NULL) {
          schemaInfo.nullable = true;
          schemaInfo.actualSchema = fieldSchema.getTypes().get(1);
        } else if (fieldSchema.getTypes().get(1).getType() == Type.NULL) {
          schemaInfo.nullable = true;
          schemaInfo.actualSchema = fieldSchema.getTypes().get(0);
        } else {
