  ...
  public static Object serialize(Object avroObject, Schema schema) {
    ...
    if (actualSchema.getType() == Type.ARRAY) {
      ...
      for (Object value : array) {
        values[i] = serialize(value, actualSchema.getElementType());
        i++;
      }
      return values;
    }

    if (actualSchema.getType() == Type.RECORD) {
      GenericRecord record = (GenericRecord) avroObject;
      List<Schema.Field> fields = actualSchema.getFields();
      Object[] values = new Object[fields.size()];
      int i = 0;
      for (Schema.Field field : fields) {
        values[i] = serialize(record.get(field.name()), field.schema());
        i++;
      }
      return values;
    }

    if (actualSchema.getType() == Type.INT) {
    ...
    if (actualSchema.getType() == Type.LONG) {
      String logicalType = actualSchema.getProp("logicalType");
    ...
    if (actualSchema.getType() == Type.DOUBLE) {
      return new DoubleWritable((Double) avroObject);
    }

    if (actualSchema.getType() == Type.BOOLEAN) {
      return new BooleanWritable((Boolean) avroObject);
    }

    if (actualSchema.getType() == Type.BYTES) {
    ...
    if (actualSchema.getType() == Type.STRING) {
