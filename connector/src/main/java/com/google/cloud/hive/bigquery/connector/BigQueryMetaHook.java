...
public class BigQueryMetaHook extends DefaultHiveMetaHook {
  ...
  public void preInsertTable(Table table, boolean overwrite) throws MetaException {
    ...
    jobInfo.setProject(tableParameters.get(Config.PROJECT.getKey()));
    jobInfo.setDataset(tableParameters.get(Config.DATASET.getKey()));
    String tableName = tableParameters.get(Config.TABLE.getKey());
    ...
    if (writeMethod.equals(RunConf.WRITE_METHOD_DIRECT)) {
      ...
      Injector injector =
          Guice.createInjector(
              new BigQueryClientModule(), new HiveBigQueryConnectorModule(conf, null));
  ...
  public void commitDropTable(Table table, boolean b) throws MetaException {
    // Do nothing
    // TODO: Drop the BigQuery table if the table is marked as "managed" (vs "external")?
  }
}
